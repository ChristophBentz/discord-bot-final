...
import { addDjRole, removeDjRole } from "./actions";

export interface DjRoleOption {
  roleId: string;
  name: string;
  color: number;
}
...
interface Props {
  current: DjRoleOption[];
  available: DjRoleOption[];
}

export function DjRoleEditor({ current, available }: Props) {
  ...
  const onAdd = (roleId: string) => {
    ...
    startTransition(async () => {
      const res = await addDjRole(roleId);
      if (res.ok) {
        setOpen(false);
        setSearch("");
        router.refresh();
      ...
      <div className="flex flex-wrap items-center gap-2">
        {current.length === 0 && (
          <span className="text-sm text-ink-muted">
            Keine DJ-Rolle gesetzt — niemand kann Musik steuern.
          </span>
        )}
        {current.map((r) => {
