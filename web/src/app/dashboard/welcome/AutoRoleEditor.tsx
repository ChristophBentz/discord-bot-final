...
import { addAutoRole, removeAutoRole } from "./actions";
...
interface Props {
  current: AutoRoleOption[];
  available: AutoRoleOption[];
}

export function AutoRoleEditor({ current, available }: Props) {
  ...
  const onAdd = (roleId: string) => {
    ...
    startTransition(async () => {
      const res = await addAutoRole(roleId);
      if (res.ok) {
        setOpen(false);
        setSearch("");
        router.refresh();
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        {current.length === 0 && (
          <span className="text-sm text-ink-muted">Keine Auto-Rolle eingerichtet.</span>
        )}
        {current.map((r) => {
